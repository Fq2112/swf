...
@section('title', 'SWF Admins: Color Categories Table | '.env('APP_TITLE'))
@push('styles')
...
@section('content')
                    ...
                    <div class="card">
                        <div class="card-header">
                            <div class="card-header-form">
                                <button onclick="createColorCategory()" class="btn btn-primary text-uppercase">
                                        ...
                                        <tr>
                                            ...
                                            <td style="vertical-align: middle" align="center">
                                                <button data-placement="left" data-toggle="tooltip" title="Edit"
                                                        ...
                                                        onclick="editColorCategory('{{$row->id}}','{{$row->name}}')">
                                                    <i class="fa fa-edit"></i></button>
                                                <a href="{{route('delete.color.categories', ['id' => encrypt($row->id)])}}"
    ...
    <div class="modal fade" id="colorCategoryModal" tabindex="-1" role="dialog"
         aria-labelledby="colorCategoryModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document" style="width: 30%">
            <div class="modal-content">
                ...
                <form id="form-colorCategory" method="post" action="{{route('create.color.categories')}}">
                    {{csrf_field()}}
...
@push("scripts")
    ...
    <script>
        $(function () {
            var export_filename = 'Color Categories Table ({{now()->format('j F Y')}})',
                table = $("#dt-buttons").DataTable({
                    ...
                    fnDrawCallback: function (oSettings) {
                        ...
                        $('.btn_massDelete').on("click", function () {
                            ...
                            $("#form-mass").attr("action", "{{route('massDelete.color.categories')}}");

                            if (ids.length > 0) {
                                swal({
                                    title: 'Delete Color Categories',
        ...
        function createColorCategory() {
            $("#colorCategoryModal .modal-title").text('Create Form');
            $("#form-colorCategory").attr('action', '{{route('create.color.categories')}}');
            $("#form-colorCategory input[name=_method]").val('');
            $("#form-colorCategory input[name=id]").val('');
            $("#form-colorCategory button[type=submit]").text('Submit');
            $("#name").val('');
            $("#colorCategoryModal").modal('show');
        }

        function editColorCategory(id, name) {
            $("#colorCategoryModal .modal-title").text('Edit Form');
            $("#form-colorCategory").attr('action', '{{route('update.color.categories')}}');
            $("#form-colorCategory input[name=_method]").val('PUT');
            $("#form-colorCategory input[name=id]").val(id);
            $("#form-colorCategory button[type=submit]").text('Save Changes');
            $("#name").val(name);
            $("#colorCategoryModal").modal('show');
        }
    </script>
